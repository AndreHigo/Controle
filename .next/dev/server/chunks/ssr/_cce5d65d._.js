...
async function createTransaction(formData) {
    ...
    const creditCardId = formData.get("credit_card_id") || null;
    const data = {
        user_id: user.id,
    ...
    if (creditCardId) {
        ...
        const { data: creditCard, error: fetchError } = await supabase.from("credit_cards").select("balance").eq("id", creditCardId).eq("user_id", user.id).single();
        if (fetchError) {
            return {
        ...
        if (type === "expense" && newBalance < 0) {
            return {
                error: "Saldo insuficiente no cartão de crédito"
            };
        }
        // Atualizar saldo do cartão
        const { error: updateError } = await supabase.from("credit_cards").update({
            balance: newBalance
        }).eq("id", creditCardId).eq("user_id", user.id);
        if (updateError) {
            return {
...
async function updateTransaction(id, formData) {
    ...
    const { data: oldTransaction, error: fetchError } = await supabase.from("transactions").select("*").eq("id", id).eq("user_id", user.id).single();
    if (fetchError) {
        return {
    ...
    if (oldTransaction.credit_card_id) {
        const { data: oldCard } = await supabase.from("credit_cards").select("balance").eq("id", oldTransaction.credit_card_id).single();
        if (oldCard) {
            const revertedBalance = oldTransaction.type === "income" ? (oldCard.balance || 0) - oldTransaction.amount : (oldCard.balance || 0) + oldTransaction.amount;
    ...
    if (creditCardId) {
        const { data: creditCard, error: fetchCardError } = await supabase.from("credit_cards").select("balance").eq("id", creditCardId).eq("user_id", user.id).single();
        if (fetchCardError) {
            return {
                error: "Erro ao buscar cartão de crédito"
            };
...
async function deleteTransaction(id) {
    ...
    const { data: transaction, error: fetchError } = await supabase.from("transactions").select("*").eq("id", id).eq("user_id", user.id).single();
    if (fetchError) {
        return {
            error: "Transação não encontrada"
        };
    ...
    if (transaction.credit_card_id) {
        const { data: creditCard } = await supabase.from("credit_cards").select("balance").eq("id", transaction.credit_card_id).single();
        if (creditCard) {
            const revertedBalance = transaction.type === "income" ? (creditCard.balance || 0) - transaction.amount : (creditCard.balance || 0) + transaction.amount;
            await supabase.from("credit_cards").update({
                balance: revertedBalance
...
"[project]/.next-internal/server/app/transactions/new/page/actions.js { ACTIONS_MODULE0 => \"[project]/app/transactions/actions.ts [app-rsc] (ecmascript)\" } [app-rsc] (server actions loader, ecmascript) <locals>", ((__turbopack_context__) => {
"use strict";
...
"[project]/.next-internal/server/app/transactions/new/page/actions.js { ACTIONS_MODULE0 => \"[project]/app/transactions/actions.ts [app-rsc] (ecmascript)\" } [app-rsc] (server actions loader, ecmascript)", ((__turbopack_context__) => {
"use strict";

__turbopack_context__.s([
    "403801f74ec77030c8c095db8308843c716787424d",
    ()=>__TURBOPACK__imported__module__$5b$project$5d2f$app$2f$transactions$2f$actions$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["createTransaction"],
    "60f00d6683e443cd12e9f8415ef52f1771e3c4791f",
    ()=>__TURBOPACK__imported__module__$5b$project$5d2f$app$2f$transactions$2f$actions$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["updateTransaction"]
]);
var __TURBOPACK__imported__module__$5b$project$5d2f2e$next$2d$internal$2f$server$2f$app$2f$transactions$2f$new$2f$page$2f$actions$2e$js__$7b$__ACTIONS_MODULE0__$3d3e$__$225b$project$5d2f$app$2f$transactions$2f$actions$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$2922$__$7d$__$5b$app$2d$rsc$5d$__$28$server__actions__loader$2c$__ecmascript$29$__$3c$locals$3e$__ = __turbopack_context__.i('[project]/.next-internal/server/app/transactions/new/page/actions.js { ACTIONS_MODULE0 => "[project]/app/transactions/actions.ts [app-rsc] (ecmascript)" } [app-rsc] (server actions loader, ecmascript) <locals>');
var __TURBOPACK__imported__module__$5b$project$5d2f$app$2f$transactions$2f$actions$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/app/transactions/actions.ts [app-rsc] (ecmascript)");
}),
];

//# sourceMappingURL=_cce5d65d._.js.map